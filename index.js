...
            text-align: center;
            width: 400px;
            border: 1px solid #334155;            transition: box-shadow 0.3s ease;
          }
...
            Live connection to Bot Process          </p>
        </div> 
...
    }).on('error', (err) => {
      console.log(`[KeepAlive] Self-pingfailed: ${err.message}`);
    });
  }, SELF_PING_INTERVAL);
...
    bot.chat(trimmed.slice(4));  } else if (trimmed.startsWith('cmd ')) {
